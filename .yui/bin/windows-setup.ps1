...
function Set-RequiredEnv {
  ...
  $targetPaths = @(
    ...
    "${env:USERPROFILE}\dotfiles\.yui\bin",
...
function Install-Shun {
  ...
  Invoke-RestMethod https://yukimemi.github.io/shun/install.ps1 | Invoke-Expression
}
  ...
  try {
    if ($PSCmdlet.ShouldProcess("System", "Run full setup")) {
      ...
      if ($isAdmin) {
        ...
        log "Not elevated, but you are in the Administrators group. Run from an elevated shell to enable Admin tasks." "Yellow"
      } else {
